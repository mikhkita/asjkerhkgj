...
//        );
        ...
        if(preg_match("/^(0[1-9]|[12][0-9]|3[01])[.](0[1-9]|1[012])[.][0-9]{2} (2[0-3]|[0-1][0-9]).[0-5][0-9]$/", $item['activeTo'])){
            ...
            $arFields = array('SITE_ID' => 's1', 'NAME' => $item['discount'], 'ACTIVE' => 'Y', 'CURRENCY' => 'RUB', 'VALUE_TYPE'=> 'P','VALUE' => $item['discount'],"PRODUCT_IDS" => array($PRODUCT_ID),'ACTIVE_TO' => $activeToDT->toString(), 'ACTIVE_FROM' => $activeFromDT->toString());
        }
        else{
            $arFields = array('SITE_ID' => 's1', 'NAME' => $item['discount'], 'ACTIVE' => 'Y', 'CURRENCY' => 'RUB', 'VALUE_TYPE'=> 'P','VALUE' => $item['discount'],"PRODUCT_IDS" => array($PRODUCT_ID));
        }
        $arFields["CONDITIONS"]["CLASS_ID"] = "CondGroup";
        $arFields["CONDITIONS"]["DATA"] = array("All" => "OR","True" => "True");

        //если продукт существует
        if ($arr = $res->Fetch()) {
            CPrice::Update($arr["ID"], $arPrice);
            if($item['discount'] > 0){
                //проверить равна ли новая и старая скидки
                //получить скидки в которых есть данный товар
                $res = CCatalogDiscount::GetDiscountProductsList(array(), array("PRODUCT_ID" => $PRODUCT_ID), false, false, array());  
                if($ob = $res->GetNext()){
                    //получить скидку товара
                    $dbCurrentDiscounts = CCatalogDiscount::GetList(array("SORT" => "ASC"), array('ID' => $ob["DISCOUNT_ID"]), false, false,array());
                    if($arCurrentDiscounts = $dbCurrentDiscounts->Fetch()){
                        //если новая скидка и старая не равны
                        if(floatval($item['discount']) != floatval($arCurrentDiscounts["VALUE"])){
                            //проверить существует ли новая скидка
                            $dbNewDiscounts = CCatalogDiscount::GetList(array("SORT" => "ASC"), array('VALUE_TYPE'=> 'P','VALUE' => $item['discount']), false, false,array());
                            if($arNewDiscounts = $dbNewDiscounts->Fetch()){//если существует
                                //обновить новую скидку
                                $addElement = array(
                                    "CLASS_ID" => "CondIBElement",
                                    "DATA" => array("logic" => "Equal", "value" => $PRODUCT_ID)
                                );
                                //получить товары, которые уже прикреплены к скидке
                                if($res = CCatalogDiscount::GetDiscountProductsList(array(), array("DISCOUNT_ID" => $arNewDiscounts["ID"]), false, false, array())){
                                    while($obNew = $res->GetNext()){
                                        $nextElement = array(
                                            "CLASS_ID" => "CondIBElement",
                                            "DATA" => array("logic" => "Equal", "value" => $obNew["PRODUCT_ID"])
                                        );
                                        $arFields["CONDITIONS"]["CHILDREN"][] = $nextElement;
                                    }
                                }
                                //добавляем новый товар
                                $arFields["CONDITIONS"]["CHILDREN"][] = $addElement;
                                CCatalogDiscount::Update($arNewDiscounts['ID'], $arFields);
                            }else{
                                //создать скидку
                                $addElement = array(
                                    "CLASS_ID" => "CondIBElement",
                            ...
                            //удалить товар из старой скидки
                            $arFields["NAME"] = $arCurrentDiscounts["NAME"];
                            $arFields["VALUE"] = $arCurrentDiscounts["VALUE"];
                            $arFields["CONDITIONS"]["CHILDREN"] = array();
                            //получить товары, которые уже прикреплены к скидке и исключить текущий товар
                            if($res = CCatalogDiscount::GetDiscountProductsList(array(), array("DISCOUNT_ID" => $ob["DISCOUNT_ID"]), false, false, array())){
                                $discountID = $ob["DISCOUNT_ID"];
                                while($obProducts = $res->GetNext()){//перебор всех товаров
                                    if($obProducts["PRODUCT_ID"] != $PRODUCT_ID){
                                        $nextElement = array(
                                            "CLASS_ID" => "CondIBElement",
                                            "DATA" => array("logic" => "Equal", "value" => $obProducts["PRODUCT_ID"])
                                        );
                                        $arFields["CONDITIONS"]["CHILDREN"][] = $nextElement;
                                    }
                                }
                                if(empty($arFields["CONDITIONS"]["CHILDREN"])){
                                    CCatalogDiscount::Delete($discountID);
                                }else{
                                    CCatalogDiscount::Update($discountID, $arFields);
                                }
                            }
                        }
                    }
                }else{
                    //получить скидку по VALUE_TYPE и VALUE
                    $dbProductDiscounts = CCatalogDiscount::GetList(array("SORT" => "ASC"), array('VALUE_TYPE'=> 'P','VALUE' => $item['discount']), false, false,array());
                    if($arProductDiscounts = $dbProductDiscounts->Fetch()){//если скидка существует
                        $addElement = array(
                            "CLASS_ID" => "CondIBElement",
                            "DATA" => array("logic" => "Equal", "value" => $PRODUCT_ID)
                        );
                        ...
                        if($res = CCatalogDiscount::GetDiscountProductsList(array(), array("DISCOUNT_ID" => $arProductDiscounts["ID"]), false, false, array())){
                            while($ob = $res->GetNext()){
                                $nextElement = array(
                                    "CLASS_ID" => "CondIBElement",
                                    "DATA" => array("logic" => "Equal", "value" => $ob["PRODUCT_ID"])
                                );
                                $arFields["CONDITIONS"]["CHILDREN"][] = $nextElement;
                            }
                        }
                    ...
                    else{//если скидка не существует
                        $addElement = array(
                            "CLASS_ID" => "CondIBElement",
                            "DATA" => array("logic" => "Equal", "value" => $PRODUCT_ID)
                        );
                ...
                //если у товара есть скидка, то удалим этот товар из скидки
                $res = CCatalogDiscount::GetDiscountProductsList(array(), array("PRODUCT_ID" => $PRODUCT_ID), false, false, array());
                $discountID = 0;
                if($ob = $res->GetNext()){
                    $arFields["CONDITIONS"]["CHILDREN"] = array();
                    //получить скидку
                    $dbProductDiscounts = CCatalogDiscount::GetList(array("SORT" => "ASC"), array('ID' => $ob["DISCOUNT_ID"]), false, false,array());
                    if($arProductDiscounts = $dbProductDiscounts->Fetch()){
                        //получить имя и величину скидки
                        $arFields["NAME"] = $arProductDiscounts["NAME"];
                        $arFields["VALUE"] = $arProductDiscounts["VALUE"];
                        //получить товары, которые уже прикреплены к скидке и исключить текущий товар
                        if($res = CCatalogDiscount::GetDiscountProductsList(array(), array("DISCOUNT_ID" => $ob["DISCOUNT_ID"]), false, false, array())){
                            $discountID = $ob["DISCOUNT_ID"];
                            while($obProducts = $res->GetNext()){//перебор всех товаров
