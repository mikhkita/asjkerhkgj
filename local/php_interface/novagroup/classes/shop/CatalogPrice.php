...
class Novagroup_Classes_General_CatalogPrice extends Novagroup_Classes_Abstract_CatalogPrice {
    ...
    function getAllPricesWithDiscount()//get all SKU, actually
    {
        $elementID = (is_array($this->elementID)) ? $this->elementID : array($this->elementID);
        $this->offersArray = CIBlockPriceTools::GetOffersArray(
            $this->catalogID
            , $elementID
            , array("ID" => "DESC")
//            , array()
...
//            ,array("STD_SIZE.ID", "COLOR.ID", "COLOR.SORT", "COLOR.PREVIEW_PICTURE","COLOR.NAME")
            , array
        (
            'STD_SIZE',
            'COLOR'
...
//        return 0;
    ...
    function getPriceByColor($colorID)    //is it usable?
    {
        $prices = array();
...
//        if(is_array($array)){
//            $min = current($array)?($array>=0):0;
//            while($cur = next($array)){
//                $min = ($cur>$min && $cur>0)?($cur):$min;
//            }
//            return $min;
//        }
//        else return 0;
