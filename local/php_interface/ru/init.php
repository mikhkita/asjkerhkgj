...
{
    if ($arFields["IBLOCK_ID"] != "19" && $arFields["IBLOCK_ID"] != "21") return;

    $exchange1cGroup = "6"; // Группа для 1С Обмена
    ...
    if ($arFields["IBLOCK_ID"] == "21" && $arFields["RESULT"] > 0) {
...
function OnAfterIBlockElementAddHandler(&$arFields) {
    ...
    if ($arFields["IBLOCK_ID"] == "13" && CModule::IncludeModule( "subscribe" ) ){
        // новости
    ...
    } elseif ($arFields["IBLOCK_ID"] == "12" && CModule::IncludeModule( "subscribe" ) ){
        // блоги
        $rsSites = CIBlock::GetSite($arFields["IBLOCK_ID"]);
        while($arSite = $rsSites->Fetch())
        {
