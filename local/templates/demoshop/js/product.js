var product = {
	...
	changeSize: function(sizeId) {
       /// $("#quickView").hide();
		...
		$('#print-button').attr("href",product.productUrl+'?size='+product.currentSizeId+'&col='+ product.currentColorId +'&print=Y');
	...
	changeStoreQuantity: function (index) {
		var f = false;
		for(var i=0;i<5;i++)//in product.currentSet[index].storeQuantity
		{
			$('#avail-count-SKLAD_'+(i+1)).html(product.currentSet[index].storeQuantity[i]);
			if(product.currentSet[index].storeQuantity[i]>0)
			{
				$('#wrapper-avail-SKLAD_'+(i+1)).css("display", "" );
				f = true;
			}
			else
			{
				$('#wrapper-avail-SKLAD_'+(i+1)).css("display", "none" );
			}
		}
		if(f)
		{
			$('.stock-availability').css("display", "" );
		}
		else{
			$('.stock-availability').css("display", "none" );
		}
	...
	changeColor: function(colorId, first) {
		...
		if (itemIndex != '') {
			...
			product.changePhotos();
			product.changeStoreQuantity(itemIndex);
	 ...
	 */
	addToSet: function(itemId, price, colorId, sizeId, colorName, colorPicture, curPhotosSmall, curPhotosBig, curPhotosBigHeight, sizeName, sizeSort, quantity, oldPrice, curPhotosMiddle,storeQuantity) {
		if (!sizeSort) sizeSort = 0;
		product.currentSet[itemId] = { 'price' : price, 'color' : colorId, 'size' : sizeId, 'colorName' : colorName, 'colorPicture' : colorPicture, 'curPhotosSmall' : curPhotosSmall, 'curPhotosBig' : curPhotosBig, 'curPhotosBigHeight' : curPhotosBigHeight, 'sizeName' : sizeName, 'sizeSort' : sizeSort, 'quantity' : quantity, 'oldPrice' : oldPrice, 'curPhotosMiddle' : curPhotosMiddle, 'storeQuantity' : storeQuantity};
	},
	 ...
	 */
	init: function (itemId, price, currentColorId, currentSizeId, colorName, colorPicture, curPhotosSmall, curPhotosBig, curPhotosBigHeight, sizeName, sizeSort, quantity, oldPrice,  ajaxUrl, productId, messages, commentsFlag, productUrl, subscribesFlag, curPhotosMiddle, detailCardView, userArr, shelvedItems,storeQuantity) {
		if (!sizeSort) sizeSort = 0;
		var self = this;
		...
		self.curStoreQuantity = storeQuantity;
